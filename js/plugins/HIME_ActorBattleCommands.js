...
If using YEP_EquipBattleSkills:  
  skill           - calls upon the equip slot of battle skills.
...
(function ($) {
  ...
  $.makeCommand_skill = function(symbol, ext) {
    return new Data_BattlerCommand(TextManager.skill, symbol);
  };
  ...
  Window_ActorCommand.prototype.addBattleCommand_skill_list = function(cmd) {
    ...
    Window_ActorCommand.prototype.addBattleCommand_skill = function(cmd) {
    var enabled = cmd.isEnabled(this._actor);
    var skillTypes = this._actor.addedSkillTypes();
    skillTypes.sort(function(a, b) {
        return a - b;
    });
        this.addCommand(cmd.name(), cmd.symbol(), enabled, 'battleSkills');
  };
